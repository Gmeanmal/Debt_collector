...
export async function resendInvitationApi(invitationId: string, email: string): Promise<void> {
  const { error, response } = await apiClient.POST("/goddess/invitations/{invitation_id}/resend", {
  ...
  if (error) {
    const status = response?.status;
    if (status === 409)
      throw Object.assign(new Error("Invitation is no longer active"), { status: 409 });
...
export async function previewInvitationApi(
  ...
  const { data, error } = await apiClient.GET("/goddess/invitations/{invitation_id}/preview", {
    params: { path: { invitation_id: invitationId } },
    headers: authHeaders(),
  });
